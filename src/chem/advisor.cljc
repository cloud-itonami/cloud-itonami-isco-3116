(ns chem.advisor
  "TestAdvisor — proposes a lab test, material analysis, or hazard-logging
  operation (record lab test data, log analysis findings, flag chemical
  safety hazards, schedule material batch review) for a registered batch. The advisor is
  ...
  batch provenance or chemical-hazard risk; `chem.governor` is the independent
  system that decides whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :draft-lab-test|:record-material-analysis|:flag-chemical-hazard|:schedule-batch-review
...
(def ^:private system-prompt
  "You are a chemical engineering technician advisor. Given a lab test, material
   analysis, or hazard-reporting operation request, propose an :op,
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "chemical operation request: " (pr-str request))}]
