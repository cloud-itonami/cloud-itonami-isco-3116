(ns chem.actor
  "ChemActor — the ISCO-08 3116 chemical engineering technician lab
  test and material analysis actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one test/analysis
  ...
  a record or dispatch a lab action the ChemGovernor refuses — every
...
(defn build-graph
  "Build a compiled ChemActor graph. `store` implements
  `chem.store/Store`. `advisor` implements `chem.advisor/Advisor`
...
(defn run-request!
  "Run one test/analysis operation request to completion or interrupt.
